...
# ╔═╡ a221b823-2387-44c8-af27-89cfeed27f09
begin
	using Pkg
	Pkg.update()
end

# ╔═╡ 39be8e09-76a7-44cb-a6d8-e3f7c7905cf1
begin
	using GLMakie
	using JLD
end

# ╔═╡ 576ea595-857b-4a5e-916c-448e92f94c29
function create_speed_matrix(velocity::Array{Float64, 4})
	return velocity[:,:,:,1].^2 + velocity[:,:,:,2].^2
end

# ╔═╡ aa2447f1-96bb-4c00-8876-2c2fc608e1a9
begin
	zeta = load("../data/zeta.jld", "zeta")
	psi = load("../data/psi.jld", "psi")
	velocity = load("../data/velocity.jld", "velocity")

	speed = create_speed_matrix(velocity)
	# u = velocity[:,:,50, 1]
	# v = velocity[:,:,50, 1]
	# speed = u.^2 + v.^2
...
# ╔═╡ 1f6540bf-12db-470e-b22b-42d152dbec1f
size(speed)

# ╔═╡ 7e4d080c-a6a3-4413-8cc6-3d85a877bc85
M, P, T = size(zeta)

# ╔═╡ 8be14982-cae0-4aa7-b581-7f176375afba
x = 1:M-1

# ╔═╡ b760ecb0-29a6-46e6-ac02-44f83a9b844c
y = 1:P-1

# ╔═╡ ea9b09f6-9c33-4b65-84b6-a8bde9629767
function create_animation(z::Array{Float64, 3}, framerate::Int, save_path::String)
	time = Observable(1)
	
	M, P, T = size(z)
	
	fig, ax, hm = heatmap(1:M, 1:P, @lift(z[:,:,$time]))
	
	framerate = 10
	timestamps = 1:T
	
	record(fig, save_path, timestamps; framerate = framerate) do t
	    time[] = t
	end
end

# ╔═╡ 7ad86ba2-d17f-457d-add2-40f76ad39c9c
create_animation(zeta, 10, "../videos/zeta_animation.mp4")

# ╔═╡ 96c7b7f3-100e-415a-bd5b-137081edea03
create_animation(speed, 10, "../videos/speed_animation.mp4")

# ╔═╡ ff0b4e13-f3fa-4644-96c9-ff38f2be6f8e
speed[:,:,5]

# ╔═╡ 4f2ec217-52a5-4380-9729-00eac135c2c2
velocity[:,:,5,1]
...
manifest_format = "2.0"
project_hash = "60222c94cf6383d5aa85306fe9030e90bbe07264"
...
[[deps.GLFW]]
deps = ["GLFW_jll"]
git-tree-sha1 = "0e09f64c12edc26f55d905b2596d28fe513e0822"
